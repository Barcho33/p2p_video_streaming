...
public class MainScreenService {
    ...
    public static boolean deletePeersMetadata(String videoId, Socket clientSocket) throws Exception {
        Sender sender = new Sender(clientSocket);
        Request request = new Request(Operations.DELETE_VIDEO_METADATA, videoId);
        sender.send(request);
    ...
    public static void showScreen(Stage primaryStage,String title, Socket clientSocket) {
        try {
            ...
            Sender sender = new Sender(clientSocket);
            sender.send(new Request(Operations.SAVE_VIDEO_METADATA, SQLiteManager.getHandshakeData()));
            Receiver receiver = new Receiver(clientSocket);
            Response response = (Response) receiver.receive();
